...
impl Apdu {
    pub fn parse_encrypted<'i>(
        ...
        let (input, apdu) =
            Self::parse(input).map_err(|_| nom::Err::Failure(Error::InvalidFormat))?;

        let apdu = match apdu {
            Apdu::GeneralGloCiphering(ciphering) => {
                let payload = ciphering
                    .decrypt(key)
        ...
        if let Some(data) = input.first() {
            match data {
                Data::Structure(data) if data.len() == 2 => {
                    if let (Data::Integer(scaler), Data::Enum(unit)) = (&data[0], &data[1]) {
...
impl Serialize for ObisMap {
        ...
        S: Serializer,
    {
        ...
        for (k, v) in self.map.iter() {
            map.serialize_entry(
                k,
                &Entry { value: v.value(), unit: v.unit().and_then(|u| u.as_str()) },
            )?;
        }
        map.end()
    }
