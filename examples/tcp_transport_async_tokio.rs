//! Asynchronous TCP transport example with Tokio runtime for DLMS/COSEM.
//!
//! This example demonstrates how to use the asynchronous TCP transport with
//! Tokio runtime to connect to a DLMS meter and perform basic operations.
//!
...
#[cfg(all(feature = "async-client", feature = "transport-tcp-async", feature = "tokio"))]
#[tokio::main]
async fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    println!("=== DLMS Async TCP Transport Example (Tokio) ===\n");
    ...
    println!("1. Creating async TCP transport with Tokio...");
    println!("   Server: {}", server_address);
    println!("   Runtime: Tokio");

    // Create async TCP transport with Tokio
    ...
    println!("API Usage Patterns (Tokio):");
    println!("----------------------------");
    println!("use tokio::main;");
    println!();
    println!("#[tokio::main]");
    println!("async fn main() -> Result<(), Box<dyn std::error::Error>> {{");
    println!("    // Create async TCP transport with Tokio");
    println!("    let mut transport = TokioTcpTransport::connect(\"192.168.1.100:4059\").await?;");
    println!();
    ...
    println!("}}");
    println!();
    println!("Benefits of Async with Tokio:");
    println!("------------------------------");
    println!("✓ High concurrency (10,000+ simultaneous connections)");
    println!("✓ Efficient resource usage (tasks instead of threads)");
    println!("✓ Non-blocking I/O (better CPU utilization)");
    println!("✓ Perfect for HES (Head-End Systems) and SaaS platforms");
    println!("✓ Rich ecosystem (tower, hyper, tonic, etc.)");
    println!();
...
#[cfg(not(all(feature = "async-client", feature = "transport-tcp-async", feature = "tokio")))]
fn main() {
    eprintln!(
        "This example requires the 'async-client', 'transport-tcp-async', and 'tokio' features."
    );
    eprintln!(
        "Run with: cargo run --example tcp_transport_async_tokio --features async-client,transport-tcp-async,tokio,encode,parse"
    );
    std::process::exit(1);
}
